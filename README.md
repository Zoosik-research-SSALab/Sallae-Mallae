...
- **Git 컨벤션 준수** — 브랜치 전략 및 커밋 메시지 규칙에 맞춰 협업 진행
- **코드 리뷰 진행** — 모든 작업을 Merge Request 기반으로 리뷰 후 병합
  - 전체 MR 433건 및 리뷰 토론 기록: [GitLab MR 코드리뷰 기록](docs/gitlab-merge-requests.md)
- **기술 문서 작성** — 빌드/배포 가이드, 외부 서비스 정보 등 기술 문서화
...
- 네이버 금융/검색 기반 KOSPI 200 뉴스 크롤러 구현 (일일 크롤러 + 체크포인트 기반 백필 크롤러)
- 17:00 KST 4단계 자동 파이프라인 스케줄러 구현 (크롤링 → 필터링 → 적재 → 신호 전송, 복구 로직 포함)
- FinBERT(KR-FinBert-SC) 기반 한국어 금융 뉴스 감성 분석 구현 및 LLM 듀얼 검증·Hard Sample 추출로 파인튜닝용 데이터셋 구축
- vLLM(Qwen2.5-7B-AWQ)/Gemini/KeyBERT 기반 키워드 추출 워커 구축 (데스크탑 GPU + DB 폴링)
- e5-small 384차원 키워드 임베딩 및 K-means 클러스터링 구현
- 임베딩 기반 기사 중복 제거 및 광고성 기사 필터링 구현
- GPU 백필 DB 적재 파이프라인 및 종목별 뉴스 에이전트 데이터 생성

🤖 AI 서버 / 에이전트 데이터

- 종목별 뉴스 에이전트 데이터 빌더 및 배치 스크립트 구현 (DB/Redis 저장)
- 뉴스 디베이트 데이터 API 및 배치 스크립트 구현

🔧 뉴스/키워드 백엔드 (BE)

- 뉴스 API 4종 구현 및 글로벌 예외 처리 정비
- 뉴스 목록 조회 기간 필터·published_at NULL 필터·totalCount 구현
- 종목 키워드 API를 agent_data JSONB + Redis 캐시 방식으로 전면 개편
- 트렌딩 키워드 Redis Sorted Set 기반 전환 및 가격/변동률/아이콘 응답 추가
- 관심종목 뉴스 API 및 뉴스 전체 기사 수 Redis 캐싱 구현

📈 메인/종목 백엔드 (BE)

- 메인 도메인 API 4종 구현
- 실시간 인기 검색 종목 TOP5 SSE API 구현
- 장중 분봉 기반 가격 갱신 스케줄러 구현
- 종목 아이콘 URL(iconUrl) 지원 및 스토리지 연동

🗄️ DB 스키마 / 데이터 모델링

- pgvector 확장 및 키워드 임베딩·클러스터링 테이블 설계
- 종목별 뉴스 에이전트 데이터 테이블, pipeline_signals(재시도 카운트) 테이블 추가
- SQL 스키마 기준 JPA Entity 동기화

🚀 인프라

- 뉴스 스케줄러 컨테이너(news-scheduler) Docker Compose 배포 설정 구성 (17:00 KST 자동 실행)
```
