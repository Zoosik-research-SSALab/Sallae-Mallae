  ...
  return (
    <section className="flex flex-col gap-5 md:gap-6">
      <div className="flex flex-col items-center gap-1 text-center">
        ...
        <p className="typo-body-md w-full text-[color:var(--color-text-secondary)]">살래말래위원회가 선정한 가장 유망한 종목입니다.</p>
      </div>
      ...
      <div className="flex flex-col gap-0">
        {featured ? (
          <div className="rounded-2xl bg-[color:var(--color-bg-tertiary)] px-6 py-5 shadow-[0px_1px_2px_rgba(0,0,0,0.05)]">
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-5">
                <span className="typo-heading-3xl text-[color:var(--color-text-interactive-primary)]">{featured.rank}</span>
                <div className="flex flex-col gap-2">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="typo-heading-sm text-[color:var(--color-text-primary)]">{featured.name}</span>
                  ...
                  <span className="typo-body-sm text-[color:var(--color-text-secondary)]">신뢰도 {featured.confidence}%</span>
                </div>
              </div>

              <div className="flex items-center gap-3">
                <div className="flex flex-col items-end gap-1">
                  <span className="typo-heading-sm text-[color:var(--color-text-primary)]">{formatPrice(featured.price)}</span>
                  <ValueChangeRateText value={featured.fluctuationRate} className={`typo-body-md font-semibold ${getRateClassName(featured.fluctuationRate)}`}>
                    {formatSignedRate(featured.fluctuationRate)}
                  </ValueChangeRateText>
                </div>
              <WatchlistHeartButton stockId={featured.stockId} stockName={featured.name} size="md" surface="muted" />
              </div>
            </div>
          </div>
        ) : null}
                    ...
                    >
                      <div className="flex items-center justify-between gap-4">
                        <div className="flex items-center gap-4">
                          <span className="typo-heading-sm w-6 text-center text-[color:var(--color-text-tertiary)]">{item.rank}</span>
                          <span className="typo-heading-sm text-[color:var(--color-text-primary)]">{item.name}</span>
                        </div>

                        <div className="flex items-center gap-1">
                          <div className="flex flex-col items-end gap-1">
                            <span className="typo-body-md font-semibold text-[color:var(--color-text-primary)]">{formatPrice(item.price)}</span>
                            <ValueChangeRateText
