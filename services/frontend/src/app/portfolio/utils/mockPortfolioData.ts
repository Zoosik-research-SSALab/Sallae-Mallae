...
const monthlyReturnsSeed = [
  { month: "2025-04", monthlyReturn: 4.2, kospiReturn: 1.1 },
  { month: "2025-05", monthlyReturn: 3.4, kospiReturn: 0.8 },
  { month: "2025-06", monthlyReturn: -1.2, kospiReturn: -2.3 },
  { month: "2025-07", monthlyReturn: 5.8, kospiReturn: 2.9 },
  { month: "2025-08", monthlyReturn: 2.1, kospiReturn: 0.5 },
  { month: "2025-09", monthlyReturn: 6.4, kospiReturn: 3.2 },
  { month: "2025-10", monthlyReturn: -0.7, kospiReturn: -1.1 },
  { month: "2025-11", monthlyReturn: 4.8, kospiReturn: 1.7 },
  { month: "2025-12", monthlyReturn: 3.9, kospiReturn: 1.1 },
  { month: "2026-01", monthlyReturn: 5.3, kospiReturn: 2.2 },
  { month: "2026-02", monthlyReturn: 4.6, kospiReturn: 1.5 },
  { month: "2026-03", monthlyReturn: 4.1, kospiReturn: 0.9 },
];
...
export function getMockPortfolioPage(): PortfolioPageData {
  return {
    ...
    monthlyReturns: monthlyReturnsSeed.map((item) => ({
      ...
      kospiReturnRate: item.kospiReturn,
      excessReturnRate: Number((item.monthlyReturn - item.kospiReturn).toFixed(1)),
    })),
    signalSummary: {
...
export function getMockPortfolioChairmanResponse(
  ...
  const holdings = holdingsSeed.map((item) => ({
    ...item,
    returnRate: Number(
      (((item.currentPrice - item.buyPrice) / item.buyPrice) * 100).toFixed(6),
    ),
  }));
  const todayTrades = todayTradesSeed.map((item) => ({
    ...
    returnRate: Number(
      (((item.currentPrice - item.tradePrice) / item.tradePrice) * 100).toFixed(6),
    ),
  }));
  const monthlyReturns = monthlyReturnsSeed.map((item) => ({
    month: item.month,
    monthlyReturn: item.monthlyReturn,
    kospiReturn: item.kospiReturn,
    alpha: Number((item.monthlyReturn - item.kospiReturn).toFixed(1)),
  }));
  ...
  const monthlyReturnsPage = monthlyReturns.slice(
    normalizedOffset,
