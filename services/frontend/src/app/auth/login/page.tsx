...
const providerButtons: ProviderButtonConfig[] = [
  ...
  },
  {
    provider: "google",
    label: "Google",
    className: "border border-[color:var(--color-border-primary)] bg-[color:var(--color-bg-primary)] text-[color:var(--color-text-primary)]",
    icon: FcGoogle,
    iconClassName: "h-6 w-6",
  },
  {
...
function LoginCard({ showCloseButton = false, onClose, onAuthenticated }: LoginCardProps) {
      ...
      <div className="flex flex-col items-center gap-4 px-5 pb-5 pt-4 sm:px-8">
            ...
            <label className="inline-flex cursor-pointer items-center gap-2">
              ...
              >
                <svg viewBox="0 0 20 20" className="h-3 w-3" fill="none" aria-hidden>
                  <path d="m4.5 10 3.2 3.2L15.5 5.4" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </span>
              <input
                type="checkbox"
        ...
        <div className="flex flex-wrap items-center justify-center gap-1 text-center">
          ...
          <Link
            href="/auth/signup"
            onClick={onClose}
            className="typo-body-sm font-semibold text-[color:var(--color-text-interactive-primary)] transition-colors hover:text-[color:var(--color-text-interactive-primary)]"
          >
            회원가입
          </Link>
        </div>
